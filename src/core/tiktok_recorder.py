...
class TikTokRecorder:
    ...
    def manual_mode(self) -> bool: # Added return type annotation
        ...
        return self.start_recording(self.user, self.room_id) # Return the stopped_by_user status
        ...
        while True:
            try:
                ...
                if self.tiktok.is_room_alive(self.room_id):
                    stopped_by_user = self.start_recording(self.user, self.room_id)
                    if stopped_by_user:
                        self.logger.info("Stopping automatic mode (User Request).")
                ...
                self.logger.info("Stopping automatic mode (KeyboardInterrupt).")
                return True # Signal main to exit

            except Exception as ex:
                self.logger.error(f"Unexpected error: {ex}\n")
        
        return False # Should not be reached (loop is infinite unless broken/returned)
    ...
    def followers_mode(self) -> bool: # Added return type annotation
        ...
        while True:
            try:
                ...
                for follower in followers:
                    ...
                    if follower in active_recordings:
                        if not active_recordings[follower].is_alive():
                            self.logger.info(f"Recording of @{follower} finished.")
                    ...
                    try:
                        ...
                        self.logger.info(f"@{follower} is live. Starting recording...")

                        thread = Thread(
                            target=self.start_recording,
                            args=(follower, room_id), # start_recording is now returning a value, which will be lost in a thread.
                                                       # The main thread needs to handle its own KeyboardInterrupt.
                            daemon=True,
                        )
                        thread.start()
                        ...
                        self.logger.error(f"Error while processing @{follower}: {e}")
                        continue

                print()
                delay = self.automatic_interval * TimeOut.ONE_MINUTE
                self.logger.info(f"Waiting {delay} minutes for the next check...")
                time.sleep(delay)
                ...
                self.logger.info("Stopping followers mode (KeyboardInterrupt).")
                return True # Signal main to exit
    ...
    def start_recording(self, user, room_id):
        ...
        # (Assume previous logic here)

        # At the end of function:
        if self.use_telegram:
            Telegram().upload(final_file)
        
        return stopped_by_user # Return status
    ...
    def start_recording(self, user, room_id):
            ...
            while not stop_recording:
                try:
                    if not self.tiktok.is_room_alive(room_id):
                        self.logger.info("User is no longer live. Stopping recording.")
                        break
                    ...
                    # If download_live_stream generator ends (e.g. stream cut), break loop
        ...
        if not VideoManagement.validate_video(output):
            self.logger.error(f"WARNING: The recorded file '{output}' appears to be corrupt or invalid.")
        ...
        if stopped_by_user:
            ...
            # Ask about re-encoding strategy ONLY if not already enforced by flag
            if self.ffmpeg_encode:
                should_encode = True
                self.logger.info("Re-encoding enforced by -ffmpeg-encode flag.")
            else:
                encode_choice = ""
                while encode_choice not in ["y", "yes", "n", "no"]:
                    encode_choice = input("Do you want to re-encode the video (slower, fixes glitches)? [y/n]: ").lower()
                should_encode = encode_choice in ["y", "yes"]

            converted_path = VideoManagement.convert_flv_to_mp4(output, should_encode)
            
            if converted_path:
                final_file = converted_path
                
                # Interactive Deletion Prompt ONLY if not enforced by --keep-flv
                if self.keep_flv:
                    self.logger.info(f"Original file '{output}' kept (per --keep-flv flag).")
                else:
                    delete_choice = ""
                    while delete_choice not in ["y", "yes", "n", "no"]:
                        delete_choice = input("Do you want to delete the original file? [y/n]: ").lower()
                    
                    if delete_choice in ["y", "yes"]:
                        try:
                            os.remove(output)
                            self.logger.info(f"Original file deleted: {output}")
                        except OSError as e:
                            self.logger.error(f"Error deleting file {output}: {e}")
        else:
            # Automatic Conversion (Stream ended naturally/Automatically)
            # Non-blocking conversion
            self.logger.info("Stream ended. Performing automatic conversion...")
            converted_path = VideoManagement.convert_flv_to_mp4(output, self.ffmpeg_encode)
            if converted_path:
                final_file = converted_path
                ...
                if not self.keep_flv:
                    try:
                        os.remove(output)
                        self.logger.info(f"Original FLV file '{output}' deleted automatically after successful conversion.")
                    except OSError as e:
                        self.logger.error(f"Error deleting original FLV file '{output}': {e}")
                else:
                     self.logger.info(f"Original FLV file '{output}' kept (per --keep-flv flag).")
