...
def main():
    ...
    # Initialize logger with default settings (verbose=False) so it's
    # available for early exceptions
    ...
    try:
        ...
        if args.verbose:
            ...
            # Refresh reference (though instance is same)
            app_logger = logger_manager_instance.logger
        ...
        if args.repair:
            ...
            result = VideoManagement.convert_flv_to_mp4(
                args.repair, ffmpeg_encode=True)
        ...
        should_exit_program = run_recordings(
            args, mode, cookies, app_logger)  # Pass the logger
