...
powershell -NoProfile -ExecutionPolicy ByPass -Command "$ErrorActionPreference='Stop'; $zip = Join-Path $env:TEMP 'tlr_download.zip'; $dest = Join-Path $env:TEMP 'tlr_download_extract'; if (Test-Path $zip) { Remove-Item $zip -Force }; if (Test-Path $dest) { Remove-Item $dest -Recurse -Force }; Invoke-WebRequest -UseBasicParsing -Uri 'https://github.com/Michele0303/tiktok-live-recorder/archive/refs/heads/main.zip' -OutFile $zip; Expand-Archive -Path $zip -DestinationPath $dest -Force; $inner = Get-ChildItem -Path $dest -Directory | Select-Object -First 1; Copy-Item -Path (Join-Path $inner.FullName '*') -Destination '%~dp0' -Recurse -Force; Remove-Item $zip -Force; Remove-Item $dest -Recurse -Force"
